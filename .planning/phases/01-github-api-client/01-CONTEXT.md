...
- **D-11a:** *(Corrected during planning from Next 16's docs — then its **premise measured false** in Phase 1. Conclusion unchanged.)* The claim was that `revalidate` alone does not cache and that caching must be opted into with `cache: 'force-cache'`.
- **D-11a-measured:** Plan 01-02 counted actual upstream requests against a local server on a production build. `{ next: { revalidate: 60 } }` alone **does** cache — 1 upstream request per 3 renders. The docs' "not cached by default" describes a fetch carrying **no** cache directive at all, which the negative control confirmed: 3 upstream requests per 3 renders. A positive `revalidate` is itself an opt-in.
  The **conclusion still stands** and `cache: 'force-cache'` ships anyway: it is the documented spelling of the intent, it survives `revalidate` later becoming `0` or conditional, and a named unit test asserts it in the outgoing init. Recorded rather than smoothed over, because the reasoning that produced D-11a was wrong even though the decision it produced was right — and a future reader deserves to know which part to trust. See `docs/OPERATIONS.md` §"Measured".
