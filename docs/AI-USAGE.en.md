...
- **Human decisions:**
  - **Approved leaving MongoDB out.** *(Attribution corrected 2026-08-02: the reasoning below was the AI's, not the developer's — the developer's contribution was agreeing with it.)* A database had been proposed so that everything could run locally. The AI argued that the app has nothing to persist — GitHub owns the data — and that local setup is already zero-service, so a database would make it worse rather than better and would read as over-engineering. The developer found that convincing, on the grounds that the app is just an API call, and approved.
