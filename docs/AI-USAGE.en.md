...
- **Human decisions:**
  - **Approved leaving MongoDB out.** *(Corrected 2026-08-02 — see Process 12. The reasoning below was the AI's, not the developer's; the developer's contribution was agreeing with it.)* A database had been proposed so that everything could run locally. The AI argued the app has nothing to persist — GitHub owns the data — that local setup is already zero-service, so a database would make it worse rather than better and would read as over-engineering. The developer found that convincing, on the grounds that the app is just an API call, and approved.
...
- **Human decisions:** This needs stating precisely, because the distinction is exactly what a reviewer is trying to assess. For this phase's implementation decisions — D-01 to D-18 in `.planning/phases/01-github-api-client/01-CONTEXT.md`, fixed before any code was written — **the AI framed the options and recommended one; the developer chose.** In every case the developer selected the option the AI had labelled *Recommended*. That covers the hybrid failure model and its rationale, the four-code vocabulary, the two cache windows, 20 results per page, `console.log` over a logging library, and logging the missing-token notice to the server rather than showing it to the user. The developer endorsed that reasoning rather than originating it, and saying the developer "decided" those points would be true but incomplete — an interviewer asking "why hybrid?" should know whose argument they are hearing.
  **The decisions that were genuinely the developer's own, unprompted, are these** — and they are the stronger evidence, because they shaped the space the options were drawn from rather than picking within it:
  - *(An earlier version of this list opened with "Rejecting MongoDB". That was wrong and is corrected in Process 12: the argument for dropping the database was the AI's and the developer approved it, which makes it an endorsement — the category directly above — not an unprompted decision. It is removed from this list rather than reworded, because the whole point of this list is that it contains only what the developer originated.)*
...
## Process 12: Correcting a misattributed decision, and documenting the workflow that produced the project

- **Date:** 2026-08-02
- **Tool:** Claude Opus 5 (1M context)
- **Delegated to AI:** Corrected the MongoDB attribution in four places (`README.md` ×2 languages, this file and its Japanese twin ×2 locations each); added three README sections in both languages covering the GSD Pi workflow and why it was used, the Superpowers skills as a complement, and unimplemented automation ideas (Jira/Notion, an AI code reviewer).
- **Why this approach:**
  - **The correction was the point, and it was initiated by the developer.** The log claimed rejecting MongoDB was a decision "genuinely the developer's own, unprompted", in a list explicitly introduced as "the stronger evidence, because they shaped the space the options were drawn from". The developer read that and said it was not true: the AI raised the argument for dropping the database and they simply agreed, "because it was just an api call". Left alone, the single most flattering claim in the log about the human's contribution would have been the false one — and an interviewer asking "so why no database?" would have got an answer that did not match the record.
  - **Removed from the list rather than reworded.** That list exists to hold only what the developer originated; a softened entry would have quietly kept credit that belongs elsewhere. A visible note explains the removal, because a silent deletion is indistinguishable from tidying.
  - **The AI did not write this section from its own summary of GSD Pi.** The developer said "look at the GSD version and the name of the github repository", so the facts were looked up rather than recalled: `@opengsd/gsd-pi`, v1.11.0 installed and current on npm, repository `open-gsd/gsd-pi`, not archived. Naming a version from memory in submission-facing documentation is precisely the kind of unfalsifiable-looking claim the log rules exist to prevent.
  - **The automation section is explicitly marked unimplemented.** Jira/Notion sync and a CodeRabbit-style reviewer loop are ideas, not features. Describing them in the same voice as the rest of the README would misrepresent what a reviewer is about to look at. Each language block opens by saying none of it is built.
  - **The automation section restates the merge boundary.** Automation would extend only to *proposing* fixes; "an agent never merges" is unaffected. Without that line, a section about automating PR fixes reads as an intention to loosen the one rule the repository is most explicit about.
- **Human decisions:**
  - **The correction itself** — spotting that the MongoDB attribution overstated their role, and saying so.
  - **Why GSD Pi was used**, given in their own selection: it forces a written plan before code, produces the audit trail the brief demands as a by-product, keeps AI work in reviewable chunks, and stays consistent across sessions.
  - **Why not Superpowers/TDD this time:** *"I just needed consistency and not a TDD method right now."* Recorded in those terms rather than dressed up as a methodological argument.
  - Asking for the automation and Superpowers sections at all.
- **Review:** `npm run lint` clean, `npm run typecheck` clean, `npm test` 243 passed — run on Node 24.18.1 despite this being a documentation change, because the rule is to run them, not to judge when they are unnecessary. Section parity confirmed by heading count: three new `###` sections in each language. GSD Pi facts confirmed against the live npm registry and GitHub API, not from memory. Entry-count parity: 12 in each file.
- **Taken as-is vs modified:** The developer's phrasing of the MongoDB correction was ambiguous on first reading — it could have meant they recommended MongoDB, or that they merely approved dropping it. Rather than pick the reading that made a better story, the AI asked, and the developer confirmed: *AI proposed dropping it; the human approved.* That is what is now recorded. This is the second time the "ask, don't invent" rule has caught an attribution error in this file, which is an argument for the rule rather than against the log.
