/**
 * `<RepoRateLimitPanel>` — the rate-limit state renderer for the detail route.
 *
 * Distinct from `<RateLimitPanel>` (which serves the search page): the search
 * variant tells the user how many minutes until the quota resets so they can
 * refine and try again; the detail variant names the exact reset time in
 * Tokyo local time and links back to search, because a user who arrived here
 * has already committed to one repository and the useful action is to leave
 * and come back later, not to retry in place. Two components, one per route
 * they serve, kept separate so a copy change on one page cannot silently
 * change the other.
 *
...
export function RepoRateLimitPanel({ resetAt }: Props) {
